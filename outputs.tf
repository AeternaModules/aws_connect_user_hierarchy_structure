...
output "connect_user_hierarchy_structures_hierarchy_structure" {
  ...
  value       = { for k, v in aws_connect_user_hierarchy_structure.connect_user_hierarchy_structures : k => one(v.hierarchy_structure) if v.hierarchy_structure != null && length(v.hierarchy_structure) > 0 }
}
